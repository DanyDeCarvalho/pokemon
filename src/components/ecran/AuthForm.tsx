...
export default function AuthForm({ isConnexion }: AuthFormProps) {
  ...
  const handleLogin = async (data: {
    ...
    if (isConnexion) {
      const res = await fetch("https://pokemonapi-production-b931.up.railway.app/auth/connexion", {
        method: 'POST',
        credentials: 'include',
        headers: {
    ...
    const res = await fetch('https://pokemonapi-production-b931.up.railway.app/auth/inscription', {
        method: 'POST',
      credentials: 'include',
      headers: {
      'Content-Type': 'application/json'
