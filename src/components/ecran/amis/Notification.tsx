...
export default function Notification() {
  ...
  const reponseDemandeAmis = async (id: string, statut: "ACCEPTE" | "REFUSE") => {
    try {
      const res = await fetch("http://localhost:3000/amis/reponseDemande", {
        method: "POST",
        credentials: 'include',
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div className="absolute top-3 right-3 z-50" ref={dropdownRef}>
      <Button
        ...
        className={`border-2 border-black p-2 shadow-md hover:bg-gray-100 transition-colors duration-300 ${
            notifications.length > 0 ? 'glow-pulse' : 'bg-white'
          }`}
